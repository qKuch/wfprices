...
function processStats(entries: any[]) {
  ...
  const history = atMax.map((e: any) => ({ t: e.datetime, v: e.median ?? e.avg_price ?? 0 })).filter((p: any) => p.v > 0)
  ...
  const yesterday = atMax.filter((e: any) => { const t = new Date(e.datetime).getTime(); return t >= twoDaysAgo && t < oneDayAgo })
...
async function fetchSlug(slug: string): Promise<any> {
  try {
    ...
    const live = processStats(data?.payload?.statistics_live?.['48hours'] ?? [])
    if (live.price !== null) return live
    return processStats(data?.payload?.statistics_closed?.['90days'] ?? [])
...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < slugs.length; i += BATCH) {
    ...
    const batchResults = await Promise.all(batch.map(fetchSlug))
