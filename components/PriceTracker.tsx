...
async function fetchArcanePrice(slug: string): Promise<PriceData> {
  try {
    // Try direct browser request to statistics endpoint (no custom headers = no CORS preflight)
    ...
    function getBestPrice(entries: any[]) {
      if (!entries.length) return null
      const maxRank = Math.max(...entries.map(e => e.mod_rank ?? 0))
      const atMax = entries.filter(e => (e.mod_rank ?? 0) === maxRank)
      if (!atMax.length) return null

      // Sort by datetime desc, take last 4h
      atMax.sort((a, b) => new Date(b.datetime).getTime() - new Date(a.datetime).getTime())
      const fourHoursAgo = Date.now() - 4 * 60 * 60 * 1000
      let pool = atMax.filter(e => new Date(e.datetime).getTime() >= fourHoursAgo)
      if (!pool.length) pool = atMax.slice(0, 3)

      const prices = pool
        .map(e => e.median ?? e.avg_price)
    ...
    const result = getBestPrice(live48h) ?? getBestPrice(closed90d)
