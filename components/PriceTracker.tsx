...
const GROUP_LABELS: Record<string,string> = {
  Legendary:'Legendary',Rare:'Rare',Uncommon:'Uncommon',Common:'Common',Ascension:'Ascension',Special:'Special',
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:'#16161a',border:'1px solid #2a2a2e',borderRadius:16,padding:24,maxWidth:580,width:'100%'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}>
          ...
          <div style={{textAlign:'right'}}>
            ...
            {pd.volume!=null&&<div style={{fontSize:11,color:'#666',marginTop:2}}>{pd.volume} tranzacții/48h</div>}
...
function FlipCalcInline() {
  const [buy,setBuy]=React.useState(''),[sell,setSell]=React.useState(''),[ qty,setQty]=React.useState('1')
  const b=parseFloat(buy)||0,s=parseFloat(sell)||0,q=parseInt(qty)||1
  const tax=Math.ceil(s*0.1),net=s-tax,profit=(net*q)-(b*q)
...
function CompactRow({a,pd,status,isBestInTier,isGlobalBest,onSelect,isFav,onToggleFav}:{
  a:Arcane;pd:PriceData;status:string;isBestInTier:boolean;isGlobalBest:boolean
  onSelect:()=>void;isFav:boolean;onToggleFav:()=>void
}) {
  ...
  const trendIcon=change==null?'—':change>2?'↑':change<-2?'↓':'→'
  const ratio=pd.price&&a.motes?Math.round(pd.price/a.motes):null
  const volColor=pd.volume!=null?(pd.volume>=10?'#4caf50':pd.volume>=3?'#666':'#e55'):'transparent'
  return (
    <div onClick={()=>pd.price!==null&&onSelect()}
      style={{display:'grid',gridTemplateColumns:'24px 1fr 80px 60px 70px 60px',alignItems:'center',gap:8,padding:'7px 12px',borderBottom:'1px solid #1a1a1e',cursor:pd.price!==null?'pointer':'default',fontSize:12}}
      onMouseEnter={e=>(e.currentTarget as HTMLDivElement).style.background='#1e1e22'}
      onMouseLeave={e=>(e.currentTarget as HTMLDivElement).style.background='transparent'}
    >
      <button onClick={e=>{e.stopPropagation();onToggleFav()}} style={{background:'none',border:'none',fontSize:13,cursor:'pointer',color:isFav?'#f5c518':'#333',padding:0,lineHeight:1}}>
        {isFav?'★':'☆'}
      </button>
      <div>
        ...
        <div style={{display:'flex',gap:4,alignItems:'center',marginTop:2}}>
          ...
          {pd.volume!=null&&<span style={{fontSize:10,color:volColor}}>● {pd.volume}/48h</span>}
        </div>
      </div>
      <div style={{textAlign:'right'}}>{pd.history&&pd.history.length>=2&&<Sparkline data={pd.history} w={70} h={20}/>}</div>
      <div style={{textAlign:'right',fontWeight:600,color:'#fff'}}>{pd.price!=null?`${pd.price} pt`:<span style={{color:'#444'}}>{status==='loading'?'...':'—'}</span>}</div>
    ...
    <div style={{maxWidth:1200,margin:'0 auto',padding:'2rem 1rem'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem',flexWrap:'wrap',gap:10}}>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
          <div style={{position:'relative'}}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Caută arcană..."
      ...
      <div style={{marginBottom:8}}>
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          ...
          {EVENT_FILTERS.map(t=>{const s=TIER_STYLE[t];return(
            <button key={t} onClick={()=>setFilter(t)} style={{fontSize:12,padding:'4px 12px',borderRadius:20,border:filter===t?`1px solid ${s.color}`:'1px solid #333',background:filter===t?s.bg:'transparent',color:filter===t?s.color:'#888',cursor:'pointer'}}>{t}</button>
          )})}
        </div>
      </div>
      ...
      {compact&&!groupedItems&&(
        <div style={{display:'grid',gridTemplateColumns:'24px 1fr 80px 60px 70px 60px',gap:8,padding:'6px 12px',fontSize:11,color:'#444',borderBottom:'1px solid #2a2a2e',marginBottom:4}}>
          <div/><div>Arcană</div><div style={{textAlign:'right'}}>Graf</div><div style={{textAlign:'right'}}>Preț</div><div style={{textAlign:'right'}}>Trend</div><div style={{textAlign:'right'}}>pt/mote</div>
        </div>
      )}
      ...
      {groupedItems?(
        <div style={{display:'flex',flexDirection:'column',gap:24}}>
          {groupedItems.map(({key,label,items})=>{
            ...
            const ss=isSynG?SYNDICATE_STYLE[key]:null,ts=!isSynG?TIER_STYLE[key]:null
            return(
              <div key={key}>
                ...
                {compact?(
                  <div style={{border:'1px solid #2a2a2e',borderRadius:10,overflow:'hidden'}}>
                    <div style={{display:'grid',gridTemplateColumns:'24px 1fr 80px 60px 70px 60px',gap:8,padding:'6px 12px',fontSize:11,color:'#444',borderBottom:'1px solid #2a2a2e'}}>
                      <div/><div>Arcană</div><div style={{textAlign:'right'}}>Graf</div><div style={{textAlign:'right'}}>Preț</div><div style={{textAlign:'right'}}>Trend</div><div style={{textAlign:'right'}}>pt/mote</div>
                    </div>
      ...
      {sortedItems.length===0&&<div style={{textAlign:'center',padding:'3rem',color:'#555',fontSize:14}}>Nicio arcană găsită{search?` pentru „${search}"`:''}</div>}
